...
if (process.env.NODE_ENV !== 'test') {
  ...
  if (args[0] === 'list') {
    listAvailableRollbackPoints()
      .then((points) => {
        ...
        points.forEach((point, index) => {
          console.log(`${index + 1}. ${point.filename}`)
          console.log(`   📅 Timestamp: ${point.timestamp}`)
          console.log(`   💾 Size: ${point.size}`)
          console.log(`   📊 Records: ${point.recordCount}`)
          console.log(`   ✅ Integrity: ${point.dataIntegrityVerified ? 'VERIFIED' : 'FAILED'}`)
          console.log('')
        })
      })
      .catch((error) => {
  ...
  } else if (args[0] === 'rollback' && args[1] && args[2]) {
    const filename = args[1]
    const confirmation = args[2]
    
    performDatabaseRollback(filename, confirmation)
      .then((result) => {
        if (result.success) {
    ...
    console.log('Usage:')
    console.log('  npx tsx scripts/database-rollback.ts list')
    console.log('  npx tsx scripts/database-rollback.ts rollback <filename> ROLLBACK_CONFIRMED_EMERGENCY')
  }
}