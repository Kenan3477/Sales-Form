// Simple script to check and update database templates
...
async function updateDatabaseTemplates() {
  try {
    ...
    templates.forEach(t => console.log(`  - ${t.name} (${t.templateType}) - Active: ${t.isActive}`));
    
    // Look for welcome letter template
    ...
    if (welcomeTemplate) {
      console.log('✅ Updating existing welcome template:', welcomeTemplate.name);
      ...
      console.log('❌ No welcome letter template found.');
      console.log('Available template types:', templates.map(t => t.templateType));
      
      // Try to find any template that could be a welcome letter
      const possibleTemplate = templates.find(t => t.isActive);
      if (possibleTemplate) {
        console.log('🔄 Updating the first active template instead:', possibleTemplate.name);
        
        await prisma.documentTemplate.update({
          where: { id: possibleTemplate.id },
          data: {
            htmlContent: enhancedTemplateContent,
        ...
        console.log('✅ Template converted to welcome letter successfully!');
      }
    }
