...
// Updated Wiseguys template with authentic branding
const AUTHENTIC_WISEGUYS_TEMPLATE = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
async function updateWiseguysTemplate() {
  try {
    console.log('🔄 Updating Wiseguys template with authentic branding...');
    ...
    if (wiseguysTemplate) {
      console.log('✅ Found Wiseguys template, updating...');
      
      await prisma.documentTemplate.update({
        where: { id: wiseguysTemplate.id },
        data: {
          htmlContent: AUTHENTIC_WISEGUYS_TEMPLATE,
          name: 'Wiseguys Welcome Letter - Authentic',
          description: 'Professional welcome letter with authentic Wiseguys Bournemouth branding (charcoal grey + lime green)'
        }
      });

      console.log('✅ Template updated with authentic branding!');
      
      // Also update the welcome-letter template for compatibility
      await prisma.documentTemplate.updateMany({
        where: { templateType: 'welcome-letter' },
        data: {
          htmlContent: AUTHENTIC_WISEGUYS_TEMPLATE,
          name: 'Wiseguys Welcome Letter (Main)',
          description: 'Authentic Wiseguys branded welcome letter'
        }
      });
      
      console.log('✅ Main welcome-letter template also updated!');

    } else {
      console.log('❌ Wiseguys template not found');
    }
