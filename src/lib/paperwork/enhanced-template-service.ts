...
<body>
                ...
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">{{coverage.startDate}}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">PLAN REF</span>
                            <span class="detail-value">{{agreement.referenceNumber}}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="card single-column-card">
                ...
                <div class="card-content">
                    <div class="numbered-list">
                        ...
                        <div class="numbered-item">
                            <span>Quote your plan reference {{agreement.referenceNumber}}</span>
                        </div>
                        <div class="numbered-item">
            ...
            <div class="card single-column-card">
                ...
                <div class="card-content">
                    ...
                    <ul class="guarantee-list">
                        ...
                        <li>Quote your plan reference {{agreement.referenceNumber}}</li>
        ...
        return arrayData.map((item, index) => {
          ...
          if (typeof item === 'object' && item !== null) {
            itemHtml = itemHtml.replace(/\{\{([^}]+)\}\}/g, (propMatch, propKey) => {
              ...
              const propValue = this.getNestedValue(item, propKey.trim());
  ...
  private processConditionals(html: string, data: any): string {
    ...
    while (attempts < maxAttempts) {
      ...
      if (match) {
        ...
        // Fallback: try to process any conditional found
        const anyConditionalRegex = /\{\{#if ([^}]+)\}\}([\s\S]*?)\{\{\/if\}\}/;
        const anyMatch = processedHtml.match(anyConditionalRegex);
        
        if (anyMatch) {
          const [fullMatch, condition, content] = anyMatch;
          const conditionValue = this.getNestedValue(data, condition.trim());
          const replacement = conditionValue ? content : '';
          processedHtml = processedHtml.replace(fullMatch, replacement);
        } else {
  ...
  getDefaultTemplate(templateType: string): string {
    return EnhancedTemplateService.templates[0]?.html || '';
  }
