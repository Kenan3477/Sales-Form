...
class AgentManager:
    ...
    async def create_agent(self, agent_config: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if process:
                ...
                ready = await self._wait_for_agent_ready(agent_id, timeout=10)
                
                if ready:
                    agent_data["status"] = "ready"
                    ...
                    print(f"[ERROR] Agent {agent_id} failed to become ready")
            ...
            print(f"[ERROR] Exception creating agent {agent_id}: {e}")
            return None
    
    async def _wait_for_agent_ready(self, agent_id: str, timeout: int = 10) -> bool:
        """Wait for agent to be ready for communication"""
    ...
    async def _start_agent_process(self, agent_data: Dict) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            # Start agent process using launcher
            cmd = ["python", "agent_launcher.py", config_file]
            ...
            if process.poll() is None:  # Process is running
                ...
                print(f"[ERROR] Agent process failed to start: {stderr_output}")
                return None
    ...
    async def assign_task(self, agent_id: str, task: Dict[str, Any]) -> bool:
        ...
        agent_status = self.active_agents[agent_id].get("status")
        if agent_status != "ready":
            print(f"[ERROR] Agent {agent_id} not ready (status: {agent_status})")
            return False
        ...
        try:
            ...
            print(f"[ERROR] Exception assigning task to agent {agent_id}: {e}")
            return False
    
    async def _send_task_to_agent(self, agent_id: str, task_id: str, task: Dict) -> bool:
        """Send task to agent via async socket communication"""
