...
class ASISTrueSelfModification:
    ...
    def __init__(self):
        ...
        # Import and initialize the safe implementation system
        from asis_safe_implementation_system import ASISSafeImplementationSystem
        self.safe_implementation = ASISSafeImplementationSystem()
        # Import and initialize the evolving knowledge system
        from asis_evolving_knowledge import ASISEvolvingKnowledge
        self.evolving_knowledge = ASISEvolvingKnowledge()
...
''')
        ...
        for i, improvement in enumerate(improvements):
            combined.append(f"\n# === IMPROVEMENT {i+1}: {improvement.get('bottleneck_type', improvement.get('gap_type', 'enhancement'))} ===")
            combined.append(f"# {improvement.get('explanation', 'Generated improvement')}")
            combined.append(improvement.get('improvement_code', improvement.get('enhancement_code', improvement.get('fix_code', ''))))
            combined.append("")
...
class ASISEnhancementIntegrator:
    ...
    def get_improvement_list(self) -> List[str]:
        """Get list of available improvements"""
        return [f"improvement_{i+1}" for i in range(''' + str(len(improvements)) + ''')]
...
''')
    ...
    async def integrate_knowledge_evolution(self, analysis: Dict, improvement_code: str, deployment_success: bool) -> Dict[str, Any]:
        """Integrate knowledge evolution with self-modification cycle"""
        
        print("🔬 Integrating knowledge from self-modification cycle...")
        
        try:
            # Create knowledge from analysis
            analysis_knowledge = {
                "concept": "self_modification_analysis",
                "content": {
                    "quality_score": analysis.get("overall_quality_score", 0),
                    "improvements_identified": len(analysis.get("improvement_priority", [])),
                    "analysis_timestamp": datetime.now().isoformat(),
                    "code_patterns": analysis.get("patterns_found", []),
                    "optimization_opportunities": analysis.get("optimization_opportunities", [])
                },
                "confidence": 0.9,
                "type": "self_analysis",
                "metadata": {"source": "self_modification_engine", "deployment_success": deployment_success}
            }
            
            # Integrate analysis knowledge
            analysis_integrated = await self.evolving_knowledge.integrate_new_knowledge(analysis_knowledge)
            
            # Create knowledge from improvement attempt
            improvement_knowledge = {
                "concept": "improvement_attempt",
                "content": {
                    "improvement_code": improvement_code[:500] + "..." if len(improvement_code) > 500 else improvement_code,
                    "deployment_success": deployment_success,
                    "timestamp": datetime.now().isoformat(),
                    "code_length": len(improvement_code),
                    "improvement_type": "self_modification"
                },
                "confidence": 0.8 if deployment_success else 0.4,
                "type": "improvement_attempt",
                "metadata": {"successful": deployment_success, "source": "self_modification_engine"}
            }
            
            # Integrate improvement knowledge
            improvement_integrated = await self.evolving_knowledge.integrate_new_knowledge(improvement_knowledge)
            
            # Evolve understanding based on new knowledge
            evolution_result = await self.evolving_knowledge.evolve_understanding()
            
            print(f"✅ Knowledge integration complete:")
            print(f"   📊 Analysis knowledge: {'integrated' if analysis_integrated else 'failed'}")
            print(f"   🔧 Improvement knowledge: {'integrated' if improvement_integrated else 'failed'}")
            print(f"   💡 New insights generated: {len(evolution_result.get('new_insights', []))}")
            
            return {
                "analysis_knowledge_integrated": analysis_integrated,
                "improvement_knowledge_integrated": improvement_integrated,
                "evolution_result": evolution_result,
                "knowledge_status": await self.evolving_knowledge.get_knowledge_status()
            }
    ...
    async def run_full_self_modification_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Phase 4: Knowledge Evolution Integration
            print("\n🧠 PHASE 4: Knowledge Evolution")
            knowledge_integration = await self.integrate_knowledge_evolution(analysis, improvement_code, deployment_success)
            cycle_result["knowledge_integration"] = knowledge_integration
    ...
    def get_self_modification_status(self) -> Dict[str, Any]:
        """Get comprehensive self-modification system status including knowledge evolution"""
        
        base_status = {
            "system_version": "2.1.0",
            "modification_engine": "True Self-Modification with Knowledge Evolution",
            "capabilities": {
                "code_analysis": True,
        ...
        # Add knowledge system status
        try:
            if hasattr(self, 'evolving_knowledge'):
                knowledge_status = asyncio.run(self.evolving_knowledge.get_knowledge_status())
                base_status["knowledge_system"] = {
                    "active": True,
                    "knowledge_nodes": knowledge_status.get("knowledge_graph_metrics", {}).get("total_nodes", 0),
                    "knowledge_connections": knowledge_status.get("knowledge_graph_metrics", {}).get("total_connections", 0),
                    "insights_generated": len([k for k in knowledge_status.get("knowledge_graph_metrics", {}).get("knowledge_types", {}) if "insight" in str(k)]),
                    "learning_optimizations": knowledge_status.get("learning_improvements", {}).get("total_optimizations", 0)
                }
                # Boost quality score based on knowledge integration
                knowledge_boost = min(10.0, base_status["knowledge_system"]["knowledge_nodes"] * 0.5)
                base_status["quality_score"] = min(100.0, base_status["quality_score"] + knowledge_boost)
            else:
                base_status["knowledge_system"] = {"active": False}
        except Exception as e:
            base_status["knowledge_system"] = {"active": False, "error": str(e)}
